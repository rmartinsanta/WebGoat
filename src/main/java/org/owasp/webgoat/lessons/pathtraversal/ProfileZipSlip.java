    ...
    private AttackResult processZipUpload(MultipartFile file) {
        ...
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                // Obtener la ruta canónica y validar que está dentro del directorio permitido
                String canonicalDestinationPath = f.getCanonicalPath();
                if (!canonicalDestinationPath.startsWith(tmpZipDirectory.toFile().getCanonicalPath() + File.separator)) {
                    throw new IOException("Intento de escritura fuera del directorio permitido: " + e.getName());
                }
